...
| **L2: 知識** | 4軸思考特性（外向/内向・感覚/直観・論理/感情・計画/柔軟）+ 興味領域 | Base OS スコアとして全推論の基盤に埋め込み | 業務シナリオベースの独自質問設計 |
...
ユーザーが評価: 👍 私らしい / ✏️ 私ならこう言わない / ⏭️ スキップ
    ↓
「私ならこう言わない」が蓄積されると、該当する判断次元を特定
    ↓
Priority Weight を自動調整（0.0〜1.0 にクランプ）
    ↓
次回以降の判断に反映
```
...
| 🔄 | **フィードバックループ** | 「私ならこう言わない」評価が蓄積すると該当次元の重みを自動調整。使うほど精度が向上 |
...
Property-Based Testing (Hypothesis) により、エッジケースを含む数万パターンの入力で正しさを保証しています。
