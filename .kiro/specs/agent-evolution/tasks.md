...
- [x] 21. Discussion Engine + ターン制議論
  - [x] 21.1 DiscussionEngine 実装
  ...
  - [x] 21.2 Discussion REST API エンドポイント実装
  ...
  - [x] 21.3 DiscussionEngine のプロパティテスト
...
- [x] 22. Compatibility Engine + レコメンド
  - [x] 22.1 CompatibilityEngine 実装
  ...
  - [x] 22.2 Compatibility + Recommendation REST API エンドポイント実装
  ...
  - [x] 22.3 CompatibilityEngine のプロパティテスト
...
- [x] 23. 会話ログ永続化 + エクスポート
  ...
  - [x] 23.2 Export REST API エンドポイント実装
...
- [x] 25. Frontend: AgentList + ChatThread
  - [x] 25.1 useAgents composable 実装
...
- [x] 26. Frontend: DiscussionTheater
  - [x] 26.1 useDiscussion composable 実装
  ...
  - [x] 26.3 DiscussionTheater.vue + TurnBubble.vue 実装
...
- [x] 28. プロファイル永続化と自動復元
  - [x] 28.1 profiles テーブル追加 + save_profile / get_profile_json / list_profile_ids
  - [x] 28.2 POST /profiles で DB 永続化
  - [x] 28.3 init_evolution_services() で起動時自動復元
  - _Requirements: 24.1, 24.2, 24.3, 24.4_

- [x] 29. フロントエンドページ統合
  - [x] 29.1 EvolutionView.vue 作成 (設定/チャット/ディスカッション 3タブ)
  - [x] 29.2 router/index.ts に /evolution ルート追加
  - _Requirements: 26.1, 26.2, 26.3_

- [ ] 30. 質問フロー → Evolution 自動連携
  - [x] 30.1 POST /sessions/{id}/calculate 完了時に自動で POST /profiles を呼び出す
  - [x] 30.2 ResultsDashboardView に「分身を作成」ボタン追加
  - _Requirements: 25.1, 25.2, 25.3_

- [x] 31. ペルソナレジストリ（公開承認フロー）
  - [x] 31.1 agents テーブルに visibility カラム追加 + マイグレーション
    - `ALTER TABLE agents ADD COLUMN visibility TEXT NOT NULL DEFAULT 'private'`
    - 既存レコードは全て `private` のまま保持
    - `idx_agents_visibility` インデックス作成
  - [x] 31.2 AgentManager に publish / unpublish / list_published メソッド追加
    - `publish()`: visibility を `published` に更新
    - `unpublish()`: visibility を `private` に戻す
    - `list_published()`: visibility='published' かつ is_active=1 の全レコードを返す
  - [x] 31.3 REST API エンドポイント追加
    - `POST /api/v1/evolution/agents/{agent_id}/publish`: 公開承認
    - `POST /api/v1/evolution/agents/{agent_id}/unpublish`: 非公開に戻す
    - `GET /api/v1/evolution/agents/registry`: 公開済み全ペルソナ一覧
  - [x] 31.4 models.py に PublishResponse / RegistryAgentResponse 追加

- [x] 32. 議論インサイトサマリー
  - [x] 32.1 discussion_summaries テーブル作成
    - DiscussionEngine.init_db() にテーブル追加
  - [x] 32.2 DiscussionEngine.generate_summary() 実装
    - 議論ログ全文 → LLM → key_insights / disagreements / unexpected_perspectives / actionable_suggestions
    - 生成済みサマリーを DB にキャッシュ
  - [x] 32.3 REST API エンドポイント追加
    - `POST /api/v1/evolution/discussions/{discussion_id}/summary`: 生成
    - `GET /api/v1/evolution/discussions/{discussion_id}/summary`: 取得

- [x] 33. フロントエンド: レジストリ UI
  - [x] 33.1 useAgents composable に publish / unpublish / listRegistry 追加
  - [x] 33.2 AgentList.vue を「マイ分身 / 公開ペルソナ」タブ切り替えに拡張
  - [x] 33.3 PublishConfirmDialog.vue 追加（公開確認モーダル）

- [x] 34. フロントエンド: インサイトサマリー UI
  - [x] 34.1 useDiscussion composable に generateSummary / getSummary 追加
  - [x] 34.2 DiscussionTheater.vue に議論完了後の「サマリー生成」ボタン + 表示セクション追加

- [x] 35. README + ランディングページ更新
  - [x] 35.1 README.md にペルソナレジストリ・インサイト機能の説明追加
  - [x] 35.2 landing/index.html にレジストリとインサイト機能のカードを追加

- [x] 36. DiscussionEngine リッチ化（Responses API + Function Calling）
  - [x] 36.1 _build_system_prompt をリッチプロンプト化（ProfileOutput 全体を使用）
  - [x] 36.2 route() → route_with_tools() に変更し search_memory ツール対応
  - [x] 36.3 agent_id → profile_id 解決 + エージェントごとの tool_executor 実装
  - [x] 36.4 _get_answer_summaries をユーティリティに抽出して共用

- [ ] 37. 口調プロファイリング精度向上（質問追加）
  - [ ] 37.1 Communication Tone カテゴリに質問追加
    - 文末表現の癖（「〜だね」「〜なんだが」「〜っすね」等）
    - 相槌・フィラー（「まあ」「ぶっちゃけ」「正直」等）
    - テキストの長さ傾向（短文 / 長文 / 箇条書き）
    - 絵文字・顔文字の使い方（使わない / 猫系 / w / 草）
    - 敬語崩れのタイミングと具体例文
    - 自由記述: 「普段のチャットのスクショを貼ってください」的な入力
  - [ ] 37.2 mapping_dictionary に新質問のスコアマッピング追加
  - [ ] 37.3 ProfileGenerator の communication_tone 生成ロジック拡張
  - [ ] 37.4 build_rich_system_prompt の口調セクションをより具体的に（例文付き）
  - [ ] 37.5 再プロファイリング → チャットテストで品質検証
